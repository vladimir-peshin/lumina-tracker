...
async function fixGOGMetadata() {
  console.log(`--- Fixing GOG Metadata for ${GOG_USERNAME} ---`);
  
  let fixedCount = 0;
  let page = 1;
  let totalPages = 1;

  const getGame = db.prepare('SELECT id, title, platform, year, developer FROM games WHERE LOWER(title) = ?');
  const updateMetadata = db.prepare('UPDATE games SET year = ?, developer = ? WHERE id = ?');
  ...
  do {
    ...
    for (const item of items) {
      ...
      if (existingGame) {
        ...
        // Fix metadata only for games that have GOG platform assigned
        if (platforms.includes('GOG')) {
          ...
          if (details) {
            updateMetadata.run(details.year, details.developer, existingGame.id);
            fixedCount++;
            console.log(`[Fixed] Updated ${gogGame.title}: Year -> ${details.year}, Developer -> ${details.developer}`);
          }
          await sleep(1000); 
        }
      }
  ...
  console.log(`Fixed metadata for: ${fixedCount} games`);
}

fixGOGMetadata().catch(err => {
